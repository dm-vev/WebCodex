...
pub(crate) struct MessageProcessor {
    ...
    native_tools_thread: Arc<Mutex<Option<Arc<CodexThread>>>>,
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        Self {
            ...
            config,
            thread_manager,
            native_tools_enabled: !std::env::var("CODEX_MCP_LEGACY_TOOLS").is_ok_and(|value| {
                matches!(value.as_str(), "1" | "true" | "TRUE" | "yes" | "YES")
            }),
            native_tools_thread: Arc::new(Mutex::new(None)),
    ...
    async fn handle_list_tools(
        ...
        if self.native_tools_enabled {
            match self.native_tools().await {
                Ok(thread) => match thread.list_direct_tools().await {
                    Ok(tools) => {
                        let mut rmcp_tools = Vec::with_capacity(tools.len());
                        for tool in tools {
                            match serde_json::to_value(tool)
                                .and_then(serde_json::from_value::<rmcp::model::Tool>)
                            {
                                Ok(tool) => rmcp_tools.push(tool),
                                Err(err) => {
                                    self.outgoing
                                        .send_error(
                                            id,
                                            ErrorData::internal_error(
                                                format!("failed to convert native tool: {err}"),
                                                None,
                                            ),
                                        )
                                        .await;
                    ...
                    Err(err) => {
                        self.outgoing
                            .send_error(
                                id,
                                ErrorData::internal_error(
                                    format!("failed to list native Codex tools: {err}"),
                                    None,
                                ),
                            )
                            .await;
                ...
                Err(err) => {
                    self.outgoing
                        .send_error(
                            id,
                            ErrorData::internal_error(
                                format!("failed to start native Codex tool thread: {err}"),
                                None,
                            ),
                        )
                        .await;
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        if self.native_tools_enabled {
            self.handle_native_tool_call(id, name.to_string(), arguments)
                .await;
            return;
        }
    ...
    async fn native_tools(&self) -> anyhow::Result<Arc<CodexThread>> {
        {
            let guard = self.native_tools_thread.lock().await;
        ...
        let new_thread = self
            .thread_manager
            .start_thread(self.config.as_ref().clone())
            .await?;
        let thread = new_thread.thread;
        let mut guard = self.native_tools_thread.lock().await;
        *guard = Some(Arc::clone(&thread));
        Ok(thread)
    }
    ...
    async fn handle_native_tool_call(
        ...
        let native_tools_thread = self.native_tools_thread.clone();
        let thread_manager = self.thread_manager.clone();
        let config = self.config.clone();

        task::spawn(async move {
            let thread = {
                let guard = native_tools_thread.lock().await;
                guard.as_ref().map(Arc::clone)
            };
            let thread = match thread {
                Some(thread) => thread,
                None => match thread_manager.start_thread(config.as_ref().clone()).await {
                    Ok(new_thread) => {
                        let thread = new_thread.thread;
                        let mut guard = native_tools_thread.lock().await;
                        *guard = Some(Arc::clone(&thread));
                        thread
                    }
                    Err(err) => {
            ...
            match thread.call_direct_tool(&name, arguments).await {
                Ok(result) => match serde_json::to_value(result)
                    .and_then(serde_json::from_value::<rmcp::model::CallToolResult>)
                {
                    Ok(result) => outgoing.send_response(request_id, result).await,
                    Err(err) => {
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("failed to convert native Codex tool result: {err}"),
                        )]);
                        outgoing.send_response(request_id, result).await;
                    }
                },
